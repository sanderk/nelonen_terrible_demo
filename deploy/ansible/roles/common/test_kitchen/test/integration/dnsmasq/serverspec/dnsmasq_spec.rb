...
describe 'dnsmasq' do
	...
	describe file('/etc/dnsmasq.d/upstream.conf') do
		it { should exist }
		it { should contain 'server=10.164.0.2', 'server=/sanoma.fi/10.236.128.21', 'server=/sanoma.nl/10.236.252.20' }
	end
	describe file('/etc/resolv.conf') do
		it { should exist }
		it { should contain 'search kitchen.staging.operations.aws.sanomahost.nl service.consul', host_inventory['ec2']['local-ipv4'] }
	end
...
describe 'awsbackup' do
	...
	describe file('/etc/sysconfig/aws_backup') do
		...
		it { should contain 'BACKUP_PREFIX="operations/staging/kitchen"' }
	end
